...
| 타임라인 뷰 | 전체 기능 |
| 공유 카드 (기본 템플릿) | 현재 D-Day 테마로 카드 생성 |
| 정렬/필터 | 전체 기능 |
| 다크모드 | 전체 기능 |
| 다국어 | 전체 기능 |
...
| 카테고리 특화 UI | 커플 100일 리스트, 시험 주/월 강조, 육아 개월수 | S04 상세 화면 |
| 공유 카드 프리미엄 템플릿 | 다크 계열, 럭셔리 테마 카드 | S06 공유 카드 → 템플릿 선택 |
...
| S04 카테고리 특화 UI | `!ref.watch(isProProvider)` | 블러 오버레이 + "Unlock with PRO" |
| S06 프리미엄 템플릿 | `template.isPro && !ref.watch(isProProvider)` | PRO 뱃지 표시, 탭 시 S08 이동 |

---

## 4. 잠금 UI 패턴

### 패턴 A: PRO 뱃지 (테마/템플릿 선택)

```
┌──────┐
│      │
│ [컬러]│
│      │  ← 테마/템플릿 미리보기 서클 (40x40)
└──────┘
  [PRO]   ← 우상단 뱃지 (금색, 8pt)
```

- 서클은 정상 표시 (어떤 컬러인지 볼 수 있음)
- 우상단에 금색 PRO 뱃지
- 탭 시 → S08 PRO 구매 화면으로 이동
- 분석 이벤트: `pro_purchase_tapped` (source: "theme" 또는 "share_template")

### 패턴 B: 블러 오버레이 (카테고리 특화 UI)
...
- 실제 콘텐츠가 블러 처리되어 뒤에 보임 (존재감 인지)
- 자물쇠 아이콘 + 설명 텍스트 + "Learn More" 버튼
- 버튼 탭 → S08 PRO 구매 화면
- 블러: `ImageFilter.blur(sigmaX: 3, sigmaY: 3)`
- 분석 이벤트: `pro_purchase_tapped` (source: "category_mode")
...
2. D-Day 상세에서 카테고리 특화 UI 발견
   → 블러된 콘텐츠 호기심 → "Learn More" 탭 → S08

3. 공유 카드 만들 때 프리미엄 템플릿 발견
...
| S04 상세 | 블러 오버레이 제거, 특화 UI 정상 표시 |
| S06 공유 카드 | PRO 뱃지 제거, 모든 템플릿 선택 가능 |
| S07 설정 | PRO 배너 숨김 |
