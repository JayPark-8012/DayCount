...
| F04 | 카테고리 태그 | 무료 | P0 | 기념일/커플/시험/여행/생일/육아 분류 (7종: general, anniversary, couple, exam, travel, birthday, baby) |
| F05 | 기본 테마 (6개) | 무료 | P1 | Cloud(circles), Sunset(waves), Ocean(waves), Forest(leaves), Lavender(circles), Minimal(circles) — 각 테마에 그라데이션+고유 패턴 장식 |
| F06 | 프리미엄 테마 (15개) | 유료 | P1 | Midnight(stars), Cherry(petals), Aurora(aurora), Peach(waves), Noir(aurora), Rose Gold(circles), Arctic(leaves), Ember(petals), Sage(leaves), Twilight(stars), Mocha(aurora), Ocean Deep(waves), Cotton Candy(circles), Graphite(stars), Royal(aurora) |
...
| F10 | 카테고리별 특화 UI/계산 | 무료 | P2 | 커플: 100일 단위 자동 리스트, 육아: 개월수+성장 마일스톤, 시험: D-30/D-60/D-90 카운트다운, 여행: 남은 일/주 강조, 생일: 나이 계산 |
...
| F20 | 홈 화면 위젯 | 무료 | P1 | 가장 가까운 D-Day 자동 표시, 탭 시 앱 열기, Android + iOS |
...
│                          │
│ ┌───────────────────┐    │
│ │ 📚 Final Exam  112│    │  ← 리스트 카드 (패턴 장식 포함)
│ │ 2026-06-20 days left│   │
│ └───────────────────┘    │
│ ┌───────────────────┐    │
│ │ ✈️ Tokyo Trip   44 │    │
│ │ 2026-04-15 days left│   │
│ └───────────────────┘    │
│ (... 나머지 카드)         │
│                    [+]   │
└─────────────────────────┘
...
- 앱바: 로고 + 설정(⚙️)
- 세그먼트 탭: 리스트 ↔ 타임라인 전환 (글래스 스타일)
- **히어로 카드**: 가장 가까운 미래 D-Day 우선, 없으면 가장 최근 과거 D-Day. 큰 카드, 패턴 opacity 0.12
- 카테고리 필터 칩: 전체, 기념일, 커플, 시험, 여행, 생일, 육아
- 리스트 헤더: "N개의 D-Day" + 정렬 드롭다운
- 리스트 카드: 히어로 카드와 별개로 **전체 D-Day 표시** (히어로 제외), 패턴 opacity 0.07
- 모든 카드: 테마별 그라데이션 배경 + 테마별 고유 패턴 장식 (design-tokens.md 9번 참고)
- FAB: + 새 D-Day 추가
...
| category | TEXT NOT NULL DEFAULT 'general' | general / anniversary / couple / exam / travel / birthday / baby |
...
| ID | 테마명 | 카드 배경 그라데이션 | 텍스트 | 액센트 | 패턴 |
|----|--------|---------------------|--------|--------|------|
| cloud | Cloud | #F0F0FF → #E0E0FF | #2D2D5F | #6C63FF | circles |
| sunset | Sunset | #FFF5ED → #FFE0CC | #8B4513 | #FF6B3D | waves |
| ocean | Ocean | #E6F5FF → #B8DFFF | #1A5276 | #2E86DE | waves |
| forest | Forest | #E8F5E9 → #C0E0C0 | #1B5E20 | #43A047 | leaves |
| lavender | Lavender | #F3E5F5 → #DCC0E8 | #4A148C | #9C27B0 | circles |
| minimal | Minimal | #FFFFFF → #F5F5F5 | #212121 | #6C63FF | circles |

### 프리미엄 테마 (유료, 15개)

| ID | 테마명 | 카드 배경 그라데이션 | 텍스트 | 액센트 | 패턴 |
|----|--------|---------------------|--------|--------|------|
| midnight | Midnight | #1A1A2E → #16213E | #E8E8FF | #6C63FF | stars |
| cherry | Cherry | #C62828 → #E53935 | #FFFFFF | #FFCDD2 | petals |
| aurora | Aurora | #0F2027 → #2C5364 | #43E8D8 | #43E8D8 | aurora |
| peach | Peach | #FFECD2 → #FCB69F | #5D4037 | #FF7043 | waves |
| noir | Noir | #232526 → #414345 | #FAFAFA | #FFD700 | aurora |
| rosegold | Rose Gold | #F4C4C4 → #E8A8A8 | #5D3A3A | #C97B7B | circles |
| arctic | Arctic | #E0F2F7 → #B2E0F0 | #0D3B66 | #1E88E5 | leaves |
| ember | Ember | #FF8A65 → #FF5722 | #FFFFFF | #FFE0B2 | petals |
| sage | Sage | #D5E1D5 → #B5C9B5 | #2E4A2E | #5C8A5C | leaves |
| twilight | Twilight | #2D1B69 → #1A1A4E | #D4C5FF | #B39DDB | stars |
| mocha | Mocha | #D7CCC8 → #BCAAA4 | #3E2723 | #795548 | aurora |
| ocean_deep | Ocean Deep | #0D1B2A → #1B3A4B | #A8D8EA | #48CAE4 | waves |
| cotton_candy | Cotton Candy | #F8BBD0 → #CE93D8 | #4A148C | #E040FB | circles |
| graphite | Graphite | #37474F → #546E7A | #ECEFF1 | #90A4AE | stars |
| royal | Royal | #1A237E → #283593 | #E8EAF6 | #FFD700 | aurora |
...
홈 화면에 D-Day 위젯을 배치하여 앱을 열지 않고도 남은 일수를 확인.
가장 가까운 D-Day를 자동으로 선택하여 표시.

### 위젯 타입

| 플랫폼 | 사이즈 | 표시 내용 |
|--------|--------|----------|
| Android | Small (2×1) | 이모지 + 제목(1줄) + 남은 일수 |
| Android | Medium (2×2) | 이모지 + 제목 + 남은 일수 + 날짜 + 테마 배경 |
| iOS | Small | 이모지 + 제목(1줄) + 남은 일수 |
| iOS | Medium | 이모지 + 제목 + 남은 일수 + 날짜 + 테마 배경 |

### D-Day 선택 로직

1. 미래 D-Day 중 가장 가까운 것 우선
2. 미래 D-Day가 없으면 가장 최근 과거 D-Day
3. D-Day가 0개면 "Add your first D-Day" 안내 표시
...
- 배경: 해당 D-Day의 테마 그라데이션 적용
- 모서리: 둥근 모서리 (플랫폼 기본 위젯 라운딩)
- 텍스트: 흰색 또는 테마 텍스트 컬러
- 탭: 앱 열기 (해당 D-Day 상세 화면으로 딥링크)

### 데이터 갱신

- D-Day 생성/수정/삭제 시 위젯 자동 갱신
- 자정 넘어갈 때 남은 일수 자동 갱신 (Android: WorkManager, iOS: Timeline reload)
- 앱 포그라운드 복귀 시 갱신
...
| 구분 | 기술 |
|------|------|
| Flutter 브릿지 | `home_widget` 패키지 |
| Android 위젯 | Glance API (Jetpack) 또는 RemoteViews + XML |
| iOS 위젯 | WidgetKit + SwiftUI |
| 데이터 공유 | SharedPreferences (Android) / UserDefaults + App Groups (iOS) |

### 데이터 공유 구조

Flutter 앱에서 위젯용 데이터를 SharedPreferences/UserDefaults에 JSON으로 저장:

```json
{
  "widget_dday_emoji": "💕",
  "widget_dday_title": "Our Anniversary",
  "widget_dday_date": "2024-06-15",
  "widget_dday_days": 258,
  "widget_dday_is_past": true,
  "widget_dday_theme_bg_start": "#F3E5F5",
  "widget_dday_theme_bg_end": "#DCC0E8",
  "widget_dday_theme_text": "#4A148C",
  "widget_dday_theme_accent": "#9C27B0",
  "widget_dday_id": 1
}
```

Flutter 측에서 D-Day 변경 시 이 데이터를 업데이트하고 `HomeWidget.updateWidget()` 호출.
...
| 버전 | 내용 | 예상 시기 |
|------|------|----------|
| **v1.0** | MVP 출시 (홈 화면 위젯 포함) | W3~W4 |
| **v1.1** | 유저 피드백 반영, 버그 수정, 신규 프리미엄 테마 추가 | v1.0 + 2주 |
| **v1.2** | 알림 시간 커스텀, 추가 언어 (일본어/중국어 간체) | v1.1 + 2주 |
| **v2.0** | 클라우드 백업/동기화 (Firebase Auth + Firestore) | v1.2 + 4주 |
| **v2.1** | 소셜 기능 (친구 기념일 공유), Apple Watch | 장기 |
