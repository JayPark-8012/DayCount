...
| F10 | 카테고리별 특화 UI/계산 | 무료 | P2 | 커플: 100일 기념일 마일스톤, 육아: 개월수+성장 마일스톤, 시험: 남은 주/월 강조 |
| F11 | 타임라인 뷰 | 무료 | P2 | 시간 축 위에 모든 D-Day 시각적 배치, TODAY 마커 |
| F12 | 공유 카드 이미지 생성 (기본) | 무료 | P2 | 기본 템플릿으로 이미지 생성, 갤러리 저장/SNS 공유 |
| F13 | 공유 카드 프리미엄 템플릿 | 유료 | P2 | 프리미엄 15개 템플릿 선택 |
| F20 | 공유 카드 배경 사진 | 유료 | P2 | 갤러리 사진으로 카드 배경 설정, 오버레이+그라데이션 |
| F21 | 공유 카드 프리미엄 폰트 | 유료 | P2 | Caveat, Dancing Script, Poppins, Playfair Display (4종) |
| F22 | 공유 카드 워터마크 제거 | 유료 | P2 | 무료: "DayCount" 워터마크 표시, PRO: 제거 |
...
- 워터마크: 무료 유저는 우하단 "DayCount" 표시 (12pt, SemiBold, 흰색 50%), PRO 유저는 제거
- 배경 선택: [컬러] / [📸 사진] 토글. 사진은 PRO 전용 (탭 시 S08 이동 또는 갤러리 오픈)
- 사진 배경: 검정 오버레이 30% + 하단 그라데이션 50%, 텍스트 흰색
- 템플릿 선택: 컬러 서클 21개 (무료 6 + 프리미엄 🔒 15). 사진 모드일 때 숨김
- 폰트 선택: 가로 스크롤 칩 5종 (Outfit 무료 + 4종 PRO 🔒). 각 칩은 해당 폰트로 렌더링
- 프리미엄 템플릿/폰트 탭 시 PRO 구매 유도 (S08 이동)
- Save: 갤러리에 이미지 저장
- Share: 네이티브 공유 시트 (카카오, 인스타, 기타)
- 분석 이벤트: `share_card_photo_used`, `share_card_font_changed`
...
│   │   │   └── analytics_utils.dart      # Firebase Analytics 헬퍼
│   │   └── extensions/
...
| 해금 기능 | 프리미엄 테마 15개 + 사진 배경 카드 + 프리미엄 폰트 4종 + 워터마크 제거 |
...
| share_card_photo_used | 공유 카드 사진 배경 사용 | - |
| share_card_font_changed | 공유 카드 폰트 변경 | font_name: string |
...
- [ ] 템플릿 선택 → 미리보기 반영 (21종: 무료 6 + PRO 15)
- [ ] 갤러리 저장 (권한 요청 포함)
- [ ] SNS 공유 시트 정상 동작
- [ ] 프리미엄 템플릿 잠금 표시 (PRO 뱃지)
- [ ] 배경 [컬러] / [📸 사진] 토글 동작
- [ ] 사진 배경: 무료 유저 → S08 이동, PRO → 갤러리 열기
- [ ] 사진 배경 카드: 오버레이 + 그라데이션 + 흰색 텍스트
- [ ] 사진 모드일 때 템플릿 선택 숨김
- [ ] 폰트 5종 가로 스크롤, 해당 폰트로 렌더링
- [ ] 프리미엄 폰트 잠금: 무료 → PRO 뱃지 + S08 이동
- [ ] 선택 폰트 → 카드 미리보기 즉시 반영 (제목 + 일수)
- [ ] 워터마크: 무료 유저 "DayCount" 표시, PRO 유저 제거
...
• Share Cards — Create beautiful share cards and post them on Instagram, send to friends, or save to your gallery. Choose from 21 templates and 5 fonts.
...
Unlock 15+ premium themes, photo background cards, premium fonts, and watermark-free sharing with a one-time purchase. No subscriptions, no ads — ever.
...
• 공유 카드 — 예쁜 이미지 카드를 만들어 인스타, 카카오톡, 어디든 공유하세요. 21종 템플릿, 5종 폰트.
...
15개 이상의 프리미엄 테마, 포토 카드, 프리미엄 폰트, 워터마크 제거를 1회 구매로 영구 해금. 구독 없음, 광고 없음.
